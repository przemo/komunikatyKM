...
shinyServer(function(input, output) {
  ...
  output$corrGram <- renderPlot({
    tt <- getDestCounts(ss,dfrom = input$date.from, dto = input$date.to, agg = input$notification.type)
    
    plo <- ggplot(data=tt)+geom_tile(aes(y=Var1,x=Var2,fill=pc), col="gray80")+theme_bw()+theme(axis.text.x = element_text(angle=90,hjust=1,vjust=0.5))+scale_fill_gradient("%", low = "grey80",high = "red")+ylab("")+xlab("")+theme(legend.position="right",legend.title=element_text(color = "gray50", vjust = 0))
    plo
  })
